...
assert.match(terrainBindingSource, /\[data-terrain-board\][\s\S]*?if \(selectedTerrainId\) return editSelectedTerrain/);
...
assert.match(display, /function publicTerrainCard/);
...
assert.match(display, /const trackFillers=boardVisible\?\[\.\.\.terrainCards\.map\(card=>\(\{terrainCard:card,label:"地形"\}\)\),\.\.\.featureEntries\]:featureEntries/);
assert.match(display, /publicMobTrack\(monster,battle,trackFillers\)/);
...
assert.match(display, /const bossTrack=\["doppelganger","guardian"\]\.includes\(battle\?\.bossMobTrack\?\.type\)\?battle\.bossMobTrack:null/);
assert.match(display, /const standardSlots=bossTrack\?null:\(Array\.isArray\(battle\?\.bpTrack\)\?battle\.bpTrack:\[\]\)/);
assert.doesNotMatch(display, /if\(!standardSlots&&!bossTrack\)return \{html:"",used:0\}/);
assert.match(display, /const filler=!occupied&&\(fillers\[used\]\?\.card\|\|fillers\[used\]\?\.terrainCard\)\?fillers\[used\+\+\]:null/);
assert.match(display, /if\(filler\?\.terrainCard\)cardContent=publicTerrainCard\(filler\)/);
...
assert.match(display, /const primaryFollowup=boardVisible\s*\? `\$\{mobTrack\.html\}\$\{featureSection\}`\s*: `\$\{mobTrack\.html\}\$\{heroClues\}\$\{featureSection\}\$\{mirrorPrimaryCards\}`/);
...
assert.match(displayStyles, /\.display-bp-markers\{z-index:9;left:12%\}/);
assert.match(displayStyles, /\.display-bp-marker\{width:28%\}/);
assert.doesNotMatch(display, /class="display-bp-marker"[^>]*>.*<b>/);
assert.match(displayStyles, /\.boss-sheet-token\.auto-token\{width:clamp\(18px,5cqi,58px\)/);
assert.match(displayStyles, /\.boss-sheet-token img\.token-square,\.display-bp-marker img\.token-square\{border-radius:6%\}/);
assert.match(displayStyles, /\.display-track-card,\.display-track-terrain\{position:absolute;inset:2px/);
assert.doesNotMatch(displayStyles, /display-track-card-label/);
assert.doesNotMatch(display, /display-track-card-label/);
assert.match(displayStyles, /\.display-track-terrain \.terrain-card-face\{width:100%;height:100%;aspect-ratio:auto\}/);
...
assert.match(renderConflictSource, /const primaryFollowup=boardVisible\s*\? `\$\{mobTrack\.html\}\$\{featureSection\}`\s*: `\$\{mobTrack\.html\}\$\{heroClues\}\$\{featureSection\}\$\{mirrorPrimaryCards\}`/);
...
assert.match(renderConflictSource, /const trackFillers=boardVisible\?\[\.\.\.terrainCards\.map\(card=>\(\{terrainCard:card,label:"地形"\}\)\),\.\.\.featureEntries\]:featureEntries/);
assert.match(renderConflictSource, /const mobTrack=publicMobTrack\(monster,battle,trackFillers\)/);
assert.match(renderConflictSource, /const usedFeatures=boardVisible\?Math\.max\(0,mobTrack\.used-terrainCards\.length\):mobTrack\.used/);
assert.match(renderConflictSource, /const remainingFeatures=featureEntries\.slice\(usedFeatures\)/);
